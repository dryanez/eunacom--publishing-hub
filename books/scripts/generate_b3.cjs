...
const aeePath = path.join(__dirname, 'aee_formatted_neurologia.json');
const aee = JSON.parse(fs.readFileSync(aeePath, 'utf8'));

// Sanitize option text if needed
if (aee['10.13'] && aee['10.13'][0]) {
  aee['10.13'][0].options.forEach(opt => {
    opt.text = opt.text.replace(/\s*­+11­neurologia­y­geriatria\/\s*1\/9/g, '').trim();
  });
}
if (aee['10.11'] && aee['10.11'][1]) {
  aee['10.11'][1].stem = aee['10.11'][1].stem.replace('enfermedada', 'enfermedad');
}
...
const topics = [
  // ==========================================================================
  // TEMA 10.11: ENFERMEDAD DE PARKINSON (TIER 3 · 4 PÁGINAS)
  // ==========================================================================
  {
    id: 'neuro-11',
    classId: 'neuro-11',
    ...
    diagramCode: `flow('Algoritmo Diagnóstico y Manejo Escalonado en Enfermedad de Parkinson', [
    ...
    table: {
      ...
      headers: ['Categoría Diagnóstica MDS', 'Manifestaciones Clínicas Específicas', 'Hallazgo Semiológico Clave', 'Interpretación y Conducta'],
      rows: [
        ['Bradicinesia (Signo Obligatorio)', 'Lentitud motora para iniciar y ejecutar movimientos + decremento progresivo', 'Fatiga y disminución de amplitud en golpeteo de dedos (finger tapping) y pronosupinación', 'Condición sine qua non para definir síndrome parkinsoniano; si falta, no es Parkinson'],
        ['Temblor de Reposo Cardinal', 'Frecuencia de 4 a 6 Hz, característicamente asimétrico al debut', 'Movimiento en "cuenta monedas" (pill-rolling) en manos; disminuye con la acción voluntaria', 'Presente en 70-80% de los pacientes; respeta típicamente la cabeza (a diferencia del esencial)'],
        ['Rigidez en Rueda Dentada', 'Resistencia plástica involuntaria constante en todo el rango pasivo', 'Fenómeno de Negro (resaltes sucesivos al combinar rigidez con temblor subclínico)', 'Afecta flexores y extensores; se incrementa al mover la extremidad contralateral (Froment)'],
        ['Inestabilidad Postural Tardía', 'Falla de reflejos posturales con tendencia a la retropulsión / propulsión', 'Prueba del tirón (pull test) anormal con retropulsión > 2 pasos o caída al suelo', 'Signo de etapa avanzada en EP; si ocurre en el primer año de evolución, es bandera roja'],
        ['Criterios de Apoyo (MDS)', 'Respuesta dramática y sostenida a Levodopa, disquinesias inducidas por L-Dopa', 'Pérdida asimétrica de braceo, hiposmia objetivada por test olfatorio, trastorno REM (RBD)', 'Confirman el diagnóstico de Enfermedad de Parkinson Clínicamente Establecida'],
        ['Banderas Rojas y Exclusión', 'Caídas tempranas (< 3 años), parálisis de mirada vertical, disautonomía severa precoz', 'Marcha en silla de ruedas en < 5 años, nula respuesta motora a dosis altas de L-Dopa', 'Obligan a descartar síndromes Parkinson-Plus (PSP, AMS) o causas secundarias']
      ]
    },
    severityTable: {
      title: 'Tabla 10.11B: Fluctuaciones Motoras y Síntomas No Motores',
      headers: ['Fenómeno Clínico', 'Mecanismo Fisiopatológico', 'Presentación Clínica Típica', 'Estrategia Terapéutica Recomendada'],
      rows: [
        ['Deterioro de Fin de Dosis ("Wearing-off")', 'Pérdida de terminales dopaminérgicas estriatales; acortamiento de vida media cerebral', 'Reaparición de rigidez, temblor y acinesia 2-3 h post-dosis antes de la siguiente toma', 'Fraccionar tomas de L-Dopa (acortar intervalo) · Asociar Entacapona 200 mg o Rasagilina'],
        ['Disquinesias de Pico de Dosis', 'Hipersensibilidad de receptores estriatales por estimulación dopaminérgica pulsátil', 'Movimientos coreicos o balísticos involuntarios en cabeza, tronco y extremidades en cénit', 'Reducir dosis individual de L-Dopa manteniendo frecuencia · Indicar Amantadina 100 mg c/12h'],
        ['Fenómeno On-Off Impredecible', 'Falla de amortiguación central y fluctuaciones impredecibles de niveles plasmáticos', 'Cambios bruscos en minutos entre movilidad normal y acinesia severa sin relación a toma', 'Optimizar intervalos · Apomorfina subcutánea de rescate · Considerar bomba de infusión o DBS'],
        ['Trastorno del Sueño REM (RBD)', 'Degeneración de núcleos pontinos subcoerúleos que median la atonía fisiológica REM', 'Pesadillas vívidas, gritos, puñetazos y patadas durmiendo; autolesiones o al cónyuge', 'Clonazepam 0.5-1 mg/noche o Melatonina 3-10 mg/noche · Medidas de protección en cama'],
        ['Disfunción Autonómica', 'Depósito de alfa-sinucleína en plexo entérico, ganglios simpáticos y vago', 'Hipotensión ortostática sintomática, constipación pertinaz, vejiga hiperactiva, disfunción eréctil', 'Fludrocortisona / Midodrina para ortostatismo · Polietilenglicol para constipación · Evitar anticolinérgicos']
      ]
    },
    treatmentTable: {
      title: 'Tabla 10.11C: Protocolo Farmacológico Escalonado en Enfermedad de Parkinson',
      headers: ['Fármaco / Intervención', 'Mecanismo de Acción', 'Dosis Estándar y Vía', 'Consideraciones Clínicas y Advertencias EUNACOM'],
      rows: [
        ['Levodopa / Carbidopa', 'Precursor directo de dopamina + inhibidor dopa-descarboxilasa periférica', 'Inicio: 100/25 mg c/8h vía oral; titular gradualmente hasta 200/50 mg c/6-8h', 'Gold standard sintomático. Tomar alejado de comidas proteicas. Fármaco de elección en > 65 años'],
        ['Pramipexol', 'Agonista dopaminérgico no ergolínico selective D2/D3', 'Inicio: 0.125 mg c/8h; titular semanal hasta 0.5-1.5 mg c/8h (o formulación LP 1 vez/día)', 'De elección en < 65 años. Alerta: Trastornos de control de impulsos (ludopatía, compras) y somnolencia'],
        ['Rasagilina', 'Inhibidor irreversible selectivo de Monoaminooxidasa B (MAO-B)', '0.5 a 1 mg una vez al día por la mañana', 'Monoterapia en etapas iniciales o coadyuvante en wearing-off. No requiere dieta sin tiramina estricta'],
        ['Entacapona', 'Inhibidor periférico reversible de Catecol-O-Metiltransferasa (COMT)', '200 mg por vía oral administrados conjuntamente con CADA dosis de Levodopa (máx 1600-2000 mg/día)', 'Prolonga vida media plasmática de L-Dopa. Trata el wearing-off. Advierte tinción naranja de orina'],
        ['Amantadina', 'Antagonista no competitivo NMDA / estimula liberación dopamina', '100 mg c/12-8h vía oral', 'Fármaco específico de elección para reducir disquinesias inducidas por Levodopa. Causa livedo reticularis'],
        ['Estimulación Cerebral Profunda (DBS)', 'Neuroestimulación eléctrica continua de alta frecuencia en STN o GPi', 'Implante quirúrgico estereotáxico bilateral de electrodos profundos + generador subclavio', 'Garantía GES N° 67 en pacientes con fluctuaciones motoras refractarias, sin demencia ni depresión severa']
      ]
    },
  ...
  // ==========================================================================
  // TEMA 10.12: PARKINSONISMOS SECUNDARIOS Y PARKINSON-PLUS (TIER 2 · 2-3 PÁG)
  // ==========================================================================
  {
    id: 'neuro-12',
    classId: 'neuro-12',
    ...
    table: {
      ...
      headers: ['Entidad Clínica', 'Etiología / Mecanismo', 'Presentación Motora Cardinal', 'Banderas Rojas y Signos Específicos', 'Respuesta a L-Dopa'],
      rows: [
        ['Parkinson Inducido por Fármacos', 'Bloqueo D2 estriatal (Metoclopramida, Haloperidol, Flunarizina, Risperidona)', 'Bilateral, rigidez y bradicinesia simétricas; temblor postural/acción', 'Antecedente farmacológico claro · Ausencia de hiposmia · Remisión al suspender', 'Nula / contraindicada (tratamiento: suspender fármaco causal)'],
        ['Parálisis Supranuclear Progresiva (PSP)', 'Tauopatía (4R-tau) en mesencéfalo, ganglios basales y núcleos subtalámicos', 'Rigidez axial prominente en extensión (retrocollis); marcha rígida', 'Parálisis de mirada vertical hacia abajo · Caídas tempranas hacia atrás · Signo colibrí en RM', 'Mala / nula (< 10-15% transitoria leve)'],
        ['Atrofia Multisistémica (AMS)', 'Alfa-sinucleinopatía en células gliales oligodendrogliales (inclusiones GCI)', 'Parkinsonismo rígido (AMS-P) o ataxia cerebelosa progresiva (AMS-C)', 'Disautonomía severa precoz (hipotensión ortostática grave, vejiga neurogénica) · Estridor laríngeo · Signo cruz caliente', 'Escasa / transitoria'],
        ['Degeneración Corticobasal (DCB)', 'Tauopatía cortical y subcortical asimétrica', 'Parkinsonismo marcadamente asimétrico con distonía focal fija de la mano', 'Apraxia ideomotora severa · Fenómeno de mano ajena (alien limb) · Mioclonías corticales reflejas', 'Completamente nula'],
        ['Parkinsonismo Vascular', 'Enfermedad isquémica microangiopática subcortical (infartos lacunares múltiples)', 'Parkinsonismo del tren inferior: marcha magnética a pequeños pasos, base ancha', 'Preservación del braceo en extremidades superiores · Hiperreflexia y Babinski (+) · Leucoaraiosis extensa en TAC/RM', 'Generalmente nula o muy pobre'],
        ['Enfermedad de Parkinson Idiopática', 'Pérdida neuronal nigroestriatal con cuerpos de Lewy (alfa-sinucleína)', 'Asimétrico al inicio: bradicinesia + temblor de reposo (4-6 Hz) + rueda dentada', 'Hiposmia y trastorno del sueño REM prodrómicos · Caídas sólo en etapas tardías', 'Excelente y sostenida (> 70% mejoría inicial)']
      ]
    },
    severityTable: null,
    treatmentTable: null,
  ...
  // ==========================================================================
  // TEMA 10.13: TEMBLOR ESENCIAL VS PARKINSONIANO Y DISTONÍAS AGUDAS (TIER 2)
  // ==========================================================================
  {
    id: 'neuro-13',
    classId: 'neuro-13',
    ...
    diagramCode: `flow('Algoritmo de Diagnóstico Diferencial de Temblores y Conducta en Distonía', [
    ...
    table: {
      ...
      headers: ['Característica Clínica', 'Temblor Esencial', 'Temblor Parkinsoniano', 'Temblor Cerebeloso', 'Distonía Aguda por Fármacos'],
      rows: [
        ['Condición de Activación', 'Postural y Cinético (aparece al sostener objetos o postura)', 'Reposo (aparece con extremidad completamente relajada)', 'Intencional / Cinético terminal (al aproximarse al objetivo)', 'Contracción tónica sostenida involuntaria (postura fija dolorosa)'],
        ['Distribución Anatómica', 'Bilateral, manos (manos y cabeza; cuerdas vocales)', 'Asimétrico, manos ("cuenta monedas"); respeta cabeza', 'Unilateral o bilateral en extremidades; dismetría ipsilateral', 'Focal o segmentario: cuello (tortícolis), ojos (crisis oculógira), lengua'],
        ['Frecuencia Oscilatoria', '8 a 12 Hz (rápido y fino)', '4 a 6 Hz (lento y de mayor amplitud)', '< 4 Hz (lento, grosero y ataxia)', 'No oscilatorio rítmico; postura fija espástica distónica'],
        ['Efecto del Alcohol', 'Mejoría transitoria notable (70% de los pacientes)', 'Sin efecto significativo', 'Sin efecto / puede empeorar ataxia', 'Sin efecto'],
        ['Signos Acompañantes', 'Ninguno; examen neurológico rigurosamente normal', 'Bradicinesia, rigidez en rueda dentada, marcha a pequeños pasos', 'Ataxia de la marcha, dismetría, disdiadococinesia, nistagmo', 'Dolor intenso por espasmo, angustia; sin fiebre ni rigidez generalizada'],
        ['Tratamiento de Elección', 'Propranolol (40-160 mg/día) o Primidona (25-250 mg/día)', 'Levodopa/Carbidopa o agonistas dopaminérgicos', 'Tratar causa subyacente; refractario a fármacos', 'Biperideno 2.5 a 5 mg IM o EV lento de urgencia']
      ]
    },
    severityTable: null,
    treatmentTable: null,
  ...
  // ==========================================================================
  // TEMA 10.14: ENFERMEDAD DE ALZHEIMER Y DCL (TIER 3 · 4 PÁGINAS)
  // ==========================================================================
  {
    id: 'neuro-14',
    classId: 'neuro-14',
    ...
    diagramCode: `flow('Algoritmo Diagnóstico del Deterioro Cognitivo y Manejo de Alzheimer', [
    ...
    table: {
      ...
      headers: ['Criterio / Característica', 'Envejecimiento Normal', 'Deterioro Cognitivo Leve (DCL)', 'Enfermedad de Alzheimer (Demencia)'],
      rows: [
        ['Queja Subjetiva de Memoria', 'Olvidos ocasionales de nombres; recupera información con pistas o tiempo', 'Queja constante referida por el paciente y confirmada por informante clave', 'Frecuentemente el paciente tiene anosognosia (no es consciente del déficit); la familia consulta'],
        ['Rendimiento en Test Cognitivos', 'Normal para edad y nivel de escolaridad (MMSE ≥ 27, MoCA ≥ 26)', 'Déficit objetivo demostrado en pruebas neuropsicológicas (MoCA < 26, MMSE 24-27)', 'Déficit multidominio significativo en test cognitivos (MMSE < 24, Test del Reloj alterado)'],
        ['Autonomía en Actividades de la Vida Diaria', 'Completamente preservada e independiente en todas las esferas', 'PRESERVADA: Realiza actividades instrumentales (dinero, compras, medicación; Pfeffer < 6)', 'PERDIDA: Dependencia progresiva de terceros para AVD instrumentales y básicas (Pfeffer ≥ 6)'],
        ['Perfil de Memoria Afectado', 'Dificultad leve en recuperación; almacenamiento y consolidación intactos', 'Afectación selectiva de consolidación episódica reciente (DCL amnésico)', 'Amnesia episódica anterógrada severa (olvida conversaciones recientes, citas y eventos del día)'],
        ['Tasa de Progresión Anual a Demencia', '1% a 2% por año (riesgo basal de la población general)', '10% a 15% por año evolucionan hacia demencia franca tipo Alzheimer', 'Enfermedad neurodegenerativa progresiva irreversible; sobrevida media 8-10 años'],
        ['Conducta Terapéutica Normada', 'Tranquilizar al paciente; fomentar actividad física e intelectual', 'Control cognitivo semestral, ejercicio aeróbico, dieta mediterránea; NO usar fármacos IAChE', 'Tratamiento específico con Donepezilo / Rivastigmina + Memantina (Garantía GES N° 85)']
      ]
    },
    severityTable: {
      title: 'Tabla 10.14B: Escalas Cognitivas y Batería de Causas Potencialmente Reversibles',
      headers: ['Herramienta / Examen', 'Parámetro Evaluado / Hallazgo', 'Puntos de Corte Clínicos', 'Relevancia EUNACOM / Conducta'],
      rows: [
        ['Mini-Mental State Exam (MMSE)', 'Orientación temporal/espacial, fijación, cálculo, recuerdo diferido, lenguaje', 'Normal: 27-30 · Leve: 20-23 · Moderado: 10-19 · Grave: < 10 puntos', 'Corte patológico < 24 puntos (Chile); útil para clasificar gravedad e indicar fármacos GES'],
        ['Montreal Cognitive Assessment (MoCA)', 'Visoespacial, ejecutiva, memoria, atención, concentración, lenguaje, abstracción', 'Puntuación máxima 30 puntos · Patológico: < 26 puntos (+1 si escolaridad ≤ 12 a)', 'Muy superior al MMSE para detectar Deterioro Cognitivo Leve y disfunción ejecutiva'],
        ['Test del Reloj (Clock Drawing Test)', 'Praxias constructivas, funciones ejecutivas de planificación, memoria de trabajo', 'Puntuación cuantitativa (Clox 1/2 o Shulman); dibujo circular, números y hora (11:10)', 'Se altera precozmente en la enfermedad de Alzheimer; excelente screening en atención primaria'],
        ['Perfil Tiroideo (TSH)', 'Función tiroidea (descarte de hipotiroidismo primario severo descompensado)', 'TSH elevada con T4 libre baja; mixedema, bradipsiquia, letargia', 'Causa reversible clásica de pseudodemencia; revierte tras reposición con Levotiroxina'],
        ['Vitamina B12 Plasmática', 'Niveles séricos de cobalamina (descarte de degeneración combinada subaguda)', 'Nivel sérico < 200 pg/mL (confirmar con ácido metilmalónico si borderline)', 'Demencia reversible con anemia megaloblástica y neuropatía sensitiva; reponer B12 IM'],
        ['VDRL / RPR en Suero', 'Tamizaje de Neurosífilis terciaria (paresia general del demente)', 'Reactivo en suero; obliga a punción lumbar para confirmar VDRL en LCR', 'Infección tratable del SNC; tratamiento curativo con Penicilina G Sódica endovenosa'],
        ['TAC / RM Cerebral sin Contraste', 'Descarte de lesiones estructurales quirúrgicas y cuantificación de atrofia', 'Atrofia hipocámpica bilateral en EA · Descartar hematoma subdural e hidrocefalia', 'Obligatorio en todo estudio inicial; detecta Hidrocefalia Normotensiva (Hakim-Adams)']
      ]
    },
    treatmentTable: {
      title: 'Tabla 10.14C: Protocolo Farmacológico con Inhibidores de Acetilcolinesterasa y Memantina',
      headers: ['Fármaco', 'Mecanismo de Acción', 'Indicación / Etapa Clínica', 'Dosis Estándar y Titulación', 'Efectos Adversos y Precauciones Críticas'],
      rows: [
        ['Donepezilo', 'Inhibidor reversible y selectivo de la Acetilcolinesterasa central', 'EA Leve a Moderada (MMSE 10 a 24 puntos)', 'Inicio: 5 mg/día oral nocturno por 4-6 semanas; titular a 10 mg/día', 'Náuseas, diarrea, insomnio, calambres. Solicitar ECG basal por riesgo de bradicardia'],
        ['Rivastigmina', 'Inhibidor pseudo-irreversible de Acetilcolinesterasa y Butirilcolinesterasa', 'EA Leve a Moderada (de elección si intolerancia gastrointestinal a orales)', 'Parche transdérmico: 4.6 mg/24h por 4 semanas; titular a 9.5 mg/24h (máximo 13.3 mg/24h)', 'Eritema cutáneo en sitio de aplicación (rotar sitio diario). Excelente adherencia'],
        ['Galantamina', 'Inhibidor de AChE + modulador alostérico de receptores nicotínicos', 'EA Leve a Moderada', 'Cápsulas LP: inicio 8 mg/día por 4 semanas; titular a 16 mg/día; máx 24 mg/día', 'Perfil colinérgico similar; administrar con desayuno para minimizar náuseas'],
        ['Memantina', 'Antagonista no competitivo de receptores NMDA de glutamato', 'EA Moderada a Severa (MMSE < 15 puntos) monoterapia o combinada', 'Inicio: 5 mg/día; aumentar 5 mg por semana hasta 20 mg/día (10 mg c/12h o 20 mg LP)', 'Excelente tolerancia; mareos leves, cefalea, constipación. Ajustar dosis en insuficiencia renal'],
        ['Quetiapina', 'Antipsicótico atípico (antagonista 5-HT2A y débil antagonista D2)', 'Manejo de BPSD refractario grave (alucinaciones angustiantes, agresividad)', '12.5 a 25 mg por la noche (dosis máxima habitual: 50-100 mg/día)', 'Aumenta riesgo de ACV y mortalidad en demencia. Usar tiempo mínimo indispensable']
      ]
    },
  ...
  // ==========================================================================
  // TEMA 10.15: DEMENCIAS NO ALZHEIMER: VASCULAR, LEWY Y FTD (TIER 2)
  // ==========================================================================
  {
    id: 'neuro-15',
    classId: 'neuro-15',
    ...
    table: {
      ...
      headers: ['Rasgo Diferencial', 'Demencia de Alzheimer', 'Demencia Vascular', 'Demencia por Cuerpos de Lewy', 'Demencia Frontotemporal (Pick)'],
      rows: [
        ['Edad de Inicio Típica', '> 65 años (senil tardía)', 'Cualquier edad con FRCV', '> 60-70 años', '45 a 65 años (típicamente presenil)'],
        ['Modo de Inicio y Curso', 'Insidioso, progresivo continuo y gradual sin mesetas', 'Fluctuante, deterioro en escalones (stepwise) tras eventos vasculares', 'Fluctuante circadiano de un día a otro o dentro del mismo día', 'Insidioso, cambio de personalidad progresivo'],
        ['Síntoma Clínico Cardinal', 'Amnesia episódica anterógrada (olvido de hechos recientes)', 'Disfunción ejecutiva frontal y enlentecimiento psicomotor', 'Tríada: Alucinaciones visuales + Fluctuaciones + Parkinsonismo', 'Desinhibición social, apatía, hiperoralidad y pérdida de empatía'],
        ['Examen Neurológico Físico', 'Normal en etapas iniciales y moderadas', 'Signos focales motores: paresias, Babinski (+), reflejos exaltados', 'Parkinsonismo espontáneo (rigidez, bradicinesia, fascies inexpresiva)', 'Comportamiento desinhibido; reflejos frontales arcaicos precoces'],
        ['Neuroimagen Clave', 'Atrofia hipocámpica y temporal medial bilateral', 'Infartos corticales múltiples / leucoaraiosis subcortical severa', 'Atrofia difusa leve o normal; hipometabolismo occipital en SPECT/PET', 'Atrofia lobar focal frontal y temporal anterior ("filo de cuchillo")'],
        ['Alerta Terapéutica Crítica', 'Donepezilo / Rivastigmina + Memantina (Garantía GES N° 85)', 'Control agresivo de FRCV + Aspirina (antiagregación secundaria)', 'CONTRAINDICACIÓN ESTRICTA de antipsicóticos típicos (mortalidad)', 'NO usar IAChE (empeoran conducta); usar ISRS (Trazodona/Sertralina)']
      ]
    },
    severityTable: null,
    treatmentTable: null,
...
topics.forEach((t, i) => {
  output += '  // ' + '='.repeat(74) + '\n';
  output += `  // TEMA ${t.topicLabel}: ${t.title.toUpperCase()}\n`;
  output += '  // ' + '='.repeat(74) + '\n';
  output += '  {\n';
  output += `    id: ${JSON.stringify(t.id)},\n`;
  output += `    classId: ${JSON.stringify(t.classId)},\n`;
  output += `    tier: ${t.tier},\n`;
  output += `    blockNum: ${t.blockNum},\n`;
  output += `    blockName: ${JSON.stringify(t.blockName)},\n`;
  output += `    topicLabel: ${JSON.stringify(t.topicLabel)},\n`;
  output += `    title: ${JSON.stringify(t.title)},\n`;
  output += `    perfilCode: ${JSON.stringify(t.perfilCode)},\n`;
  output += `    dx: ${JSON.stringify(t.dx)},\n`;
  output += `    tx: ${JSON.stringify(t.tx)},\n`;
  output += `    seg: ${JSON.stringify(t.seg)},\n`;
  output += `    ges: ${JSON.stringify(t.ges)},\n`;
  output += `    reconstrucciones: ${JSON.stringify(t.reconstrucciones)},\n`;
  output += `    frecuencia: ${JSON.stringify(t.frecuencia)},\n`;
  output += `    svg: ${t.svg},\n`;
  output += `    algoTitle: ${JSON.stringify(t.algoTitle)},\n`;
  if (t.diagramCode) {
    output += `    diagram: ${t.diagramCode},\n`;
  } else {
    output += `    diagram: null,\n`;
  }
  output += `    contexto: ${JSON.stringify(t.contexto)},\n`;
  output += `    contentSections: ${JSON.stringify(t.contentSections, null, 6)},\n`;
  output += `    table: ${JSON.stringify(t.table, null, 6)},\n`;
  output += `    severityTable: ${JSON.stringify(t.severityTable, null, 6)},\n`;
  output += `    treatmentTable: ${JSON.stringify(t.treatmentTable, null, 6)},\n`;
  output += `    vignette: ${JSON.stringify(t.vignette)},\n`;
  output += `    explicacion: ${JSON.stringify(t.explicacion)},\n`;
  output += `    keyPoints: ${JSON.stringify(t.keyPoints, null, 6)},\n`;
  output += `    questions: ${JSON.stringify(t.questions, null, 6)}\n`;
  output += '  }' + (i < topics.length - 1 ? ',' : '') + '\n';
});
...
const dest1 = path.join(__dirname, 'dataset_neurologia_bloque_3.cjs');
fs.writeFileSync(dest1, output, 'utf8');
console.log('Successfully written to:', dest1, '(Size:', output.length, 'bytes)');

const dest2 = path.join('d:\\Anti\\Eunacom\\eunacom-app-v2\\book\\scripts\\dataset_neurologia_bloque_3.cjs');
fs.writeFileSync(dest2, output, 'utf8');
console.log('Successfully mirrored to:', dest2);
