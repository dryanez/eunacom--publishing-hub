/**
 * ============================================================================
 * TOMO 09: REUMATOLOGÍA & INMUNOLOGÍA CLÍNICA
 ...
 * Cabecera y Color Temático: Granate (#9f1239)
 * ============================================================================
 */

const b1 = require('./dataset_reumatologia_bloque_1.cjs');
const b2 = require('./dataset_reumatologia_bloque_2.cjs');
const b3 = require('./dataset_reumatologia_bloque_3.cjs');
const b4 = require('./dataset_reumatologia_bloque_4.cjs');
const b5 = require('./dataset_reumatologia_bloque_5.cjs');

// Preguntas Oficiales AEE extraídas directamente de questionDB (6.000+ preguntas)
let aeeQuestions = {};
try {
  aeeQuestions = require('./aee_formatted_reumatologia.json');
} catch (e) {
  console.warn('aee_formatted_reumatologia.json not loaded, falling back to embedded questions');
}
...
const reumatologiaClasses = rawClasses.map(c => {
  ...
  const normQuestions = sourceQuestions.map(q => {
    ...
    return {
      ...
      recTag: q.recTag || `Banco Oficial AEE · Perfil V3 ${c.perfilCode || '1.11.1'}`
    };
  });
